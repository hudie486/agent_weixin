...
| 自然语言命令（NLU） | `NLU_ENABLE=1` 且配置 `DEEPSEEK_API_KEY` 时，非斜杠文本经**预筛缩小候选** + **DeepSeek 抽槽**映射为模块命令；缺参时多轮追问。`NLU_STYLE_ENABLE=1` 仅润色填参/消歧等交互话术；**命令执行结果（列表、成功/失败）不润色** |
...
| `/用户 help` / `验证` / `logout` / `登记` / `添加` / `列表` / `简称` / `password` / `喊话` / `通知` / `查看` / `删除` / `QQ …` | 用户中心：验证、登记、简称、向管理员喊话、管理员通知/查看/删除（目标可用**简称**或 `userId`） |
...
- 如果不是斜杠命令（且不在向导态）：若 `NLU_ENABLE=1`，先 `tryDispatchNluText`（预筛 + DeepSeek 抽槽 → 命令或填参追问）；未命中或 NLU 关闭时走 `agent` 模块的 `chat` 通路。
...
- **NLU**：`NLU_ENABLE`、`DEEPSEEK_API_KEY`、`NLU_STYLE_ENABLE`（仅润色 NLU 交互话术）、`NLU_LLM_MODEL`、`NLU_CONFIDENCE_MIN`、`NLU_INTERRUPT_MIN`、`INTERACTION_STATE_PATH`
