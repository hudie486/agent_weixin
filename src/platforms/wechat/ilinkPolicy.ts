/**
 * 微信 iLink 私聊出站策略：24 小时窗口、连续 10 条限制等。
 * 仅由 platforms/wechat/ilinkSend 使用；转发层不直接引用。
 */
...
export type WechatIlinkPolicyConfig = {
  windowMs: number;
  maxConsecutive: number;
  warnBeforeMs: number;
};

export function loadWechatIlinkPolicyConfig(): WechatIlinkPolicyConfig {
...
export type WechatIlinkGateResult = {
  allow: boolean;
...
export function gateWechatIlinkOutbound(args: {
  proactive: boolean;
  state: { lastInboundAt: number; consecutiveBotMessages: number };
  nowMs: number;
  cfg: WechatIlinkPolicyConfig;
}): WechatIlinkGateResult {
...
export function withWechatIlinkHints(text: string, appendLimit: boolean, appendWindow: boolean): string {
