...
export function registerUserCommandSystem(catalog: CommandCatalog): void {
  ...
  const defs: CommandDescriptor[] = [
    ...
    },
    {
      ...
      action: "list",
      keywords: ["列表"],
      nluHints: ["有哪些用户", "那些用户", "当前用户", "有谁", "用户列表"],
      wizardMenuLabel: "列表",
      usage: "/用户 列表",
    ...
    },
    {
      ...
      action: "shortname",
      keywords: ["简称"],
      nluHints: ["设置简称", "设置我的简称", "我的简称", "改简称", "称呼"],
      wizardMenuLabel: "简称",
      usage: "/用户 简称 <名称>",
      summary: "设置本人在对话与命令中的简称",
      params: [
        {
          name: "shortName",
          label: "简称",
          prompt: "请输入你的简称（2～24 字，全局唯一）：",
          kind: "text",
          required: true,
        },
      ],
      buildSub: (c) => `简称 ${c.shortName ?? ""}`.trim(),
    },
    {
      domain: "user",
      action: "call",
      keywords: ["喊话"],
      nluHints: ["向管理员喊话", "给管理员发", "管理员喊话"],
      wizardMenuLabel: "喊话",
      usage: "/用户 喊话 <内容>",
      summary: "向管理员发送消息（不是发给普通用户；发给用户请用「通知」）",
      params: [
        {
          name: "text",
          label: "内容",
    ...
    },
    {
      ...
      wizardMenuLabel: "通知",
      usage: "/用户 通知 <userId|简称> <内容>",
      summary: "管理员向指定用户发消息（可用简称）",
      requiresAdmin: true,
      params: [
        {
          name: "userId",
          label: "目标用户",
          prompt: "请指定要通知的用户（简称或 userId）：",
          kind: "userId",
          required: true,
        },
