...
export type ActionResolvers = Partial<
  Record<"periodic" | "code" | "env" | "user" | "qq", ResolveActionFn>
>;
...
export async function routeSlashCommand(
  ...
  const allowed = new Set(["periodic", "code", "env", "user", "qq"]);
  if (!allowed.has(routed.domain)) {
    return false;
  }
