...
function buildSystemPrompt(manifests: NluCommandManifest[], domainHints: string[] = []): string {
  ...
  return [
    ...
    "- periodic.create：description=任务需求全文（含 URL，勿截断）；kind=schedule|trigger（未说则省略）；cronExpression=仅当用户明确说了时间（5 段或「每天9点」）；shortName=仅当用户给了简称；不要把整句塞进 rest，不要臆造 confirm",
    ...
    '- 「创建周期任务，任务内容参考：https://github.com/parleychou/GlmGrap.git」→ {"intent":{"domain":"periodic","action":"create","slots":{"description":"参考 https://github.com/parleychou/GlmGrap.git"},"confidence":0.9}}',
